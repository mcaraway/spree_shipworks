module Spree
  module Shipworks
    class EmailInvoices
      VALID_STATES          = %w(ready canceled resumed awaiting_return returned)
      VALID_SHIPMENT_STATES = ::Spree::Shipment.state_machine.events.collect(&:name)
      def self.since(start_date = nil)
        scope = Spree::EmailInvoice.
        where(:shipping_state => VALID_STATES).
      ...
      def self.since_in_batches(start_string, maxcount_string)
        ...
        begin
          date = DateTime.parse(start_string.sub("T", " "))
        rescue
        ...
        Spree::EmailInvoice.uncached do
          orders = relation.offset(batch_size * batch).load
          while orders.any?
            orders.each do |order|
              ...
              order.extend(Xml::EmailInvoice)
              last_updated_at = order.updated_at
              yield order
            end
            break if orders.size < batch_size || broken
            orders = relation.offset(batch_size * (batch += 1)).load
          end
        end
      end
    end
