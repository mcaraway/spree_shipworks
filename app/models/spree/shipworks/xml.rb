  ...
  module Shipworks
    module Xml
      ...
      module Order
        def to_shipworks_xml(context)
          context.element 'Order' do |order_context|
            ...
            order_context.element 'ShippingMethod', self.shipments.first.shipping_methods.first.try(:name)
            ...
            # if self.payments.first.present?
              # payment = self.payments.first.extend(::Spree::Shipworks::Xml::Payment)
      ...
      module EmailInvoice
        def to_shipworks_xml(context)
          context.element 'Order' do |order_context|
            order_context.element 'OrderNumber',    self.order_number
            order_context.element 'OrderDate',      self.created_at.to_s(:db).gsub(" ", "T")
            order_context.element 'LastModified',   self.updated_at.to_s(:db).gsub(" ", "T")
            order_context.element 'ShippingMethod', self.shipping_method
            order_context.element 'StatusCode',     self.shipping_state
            order_context.element 'CustomerID',     self.user.try(:id)
  
            # if ship_address = self.get_shipping_address
              # ship_address.extend(Address)
              # ship_address.to_shipworks_xml('ShippingAddress', order_context)
              order_context.element 'ShippingAddress' do |a|
                a.element 'FullName',   self.full_name
                a.element 'Street1',    self.address
                a.element 'Street2',    ''
                a.element 'City',       self.city
                a.element 'State',      Spree::State.where("name = ?",self.state).first.try(:abbr)
                a.element 'PostalCode', self.zip
                a.element 'Country',    self.country.try(:iso_name)
                a.element 'Phone',      self.phone
            ...
            # end
              ...
              order_context.element 'BillingAddress' do |a|
                a.element 'FullName',   self.full_name
                a.element 'Street1',    self.address
                a.element 'Street2',    ''
                a.element 'City',       self.city
            ...
            # if bill_address = self.get_shipping_address
            ...
            # if self.payments.first.present?
              ...
              # payment.to_shipworks_xml(order_context)
            # end
            line_items = self.email_invoice_items
            order_context.element 'Items' do |items_context|
              line_items.each do |item|
                item.extend(EmailInvoiceLineItem)
                item.to_shipworks_xml(items_context) 
              end
            end if line_items.present?
  
            order_context.element 'Totals' do |totals_context|
              totals_context.element 'Total', format("%01.2f", 0.0),
                                      :id => 1,
                                      :name => "total",
                                      :impact => "add"
            end
      ...
      module EmailInvoiceLineItem
        def to_shipworks_xml(context)
          context.element 'Item' do |i|
            i.element 'ItemID',    self.id             
            i.element 'ProductID', self.sku                                         
            i.element 'Code',      self.sku                                                                       
            i.element 'SKU',       self.sku                                       
            i.element 'Name',      self.name
            i.element 'Quantity',  self.count
            i.element 'UnitPrice', format("%01.2f", 0.0)
            i.element 'UnitCost',  format("%01.2f", 0.0)              
            i.element 'Weight',    0.0                             
          end
        end
      end # EmailInvoiceLineItem
